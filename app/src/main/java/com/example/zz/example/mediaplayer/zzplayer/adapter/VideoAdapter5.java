package com.example.zz.example.mediaplayer.zzplayer.adapter;
...
import com.example.zz.example.mediaplayer.zzplayer.utils.MyFormatter;
 ...
 */
public class VideoAdapter5 extends RecyclerView.Adapter {
    //    List<VideoBean> videoBeans = new ArrayList<>();
    Cursor videoCursor;
    Context context;

    public VideoAdapter5(Context context) {
...
//        return videoBeans == null ? 0 : videoBeans.size();
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        videoViewHolder.sizeTv.setText(Formatter.formatFileSize(context, videoBean.getSize()));
        videoViewHolder.durationTv.setText(MyFormatter.formatDuration(videoBean.getDuration()));


        String thumbPath = thumbsMap.get(videoBean.getId());
        if (thumbPath == null) {
            videoViewHolder.iv.setImageResource(R.mipmap.ic_launcher);
        } else {
